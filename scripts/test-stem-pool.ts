...
// ── empty pool ──
check('empty pool reads as {stems:[]}', readPool('leo').stems.length === 0);
check('empty pool status = 0/0/0', JSON.stringify(poolStatus('leo')) === JSON.stringify({ free: 0, leased: 0, total: 0 }));
check('checkout on empty pool → null (floor trigger)', checkoutStem('leo', NOW) === null);

// ── populate N ──
const N = 3;
for (let i = 0; i < N; i++) upsertStem('leo', stem(i));
check(`upsert ${N} stems → ${N} free`, poolStatus('leo').free === N);
check('upsert is idempotent by stem_id (re-upsert stem 0)', (() => { upsertStem('leo', stem(0)); return poolStatus('leo').total === N; })());

// ── checkout N distinct, (N+1)th null ──
const leased = new Set<string>();
for (let i = 0; i < N; i++) { const s = checkoutStem('leo', NOW); if (s) leased.add(s.stem_id); }
check(`checkout ${N}× → ${N} DISTINCT leases (no double-lease)`, leased.size === N);
check('all leased → 0 free', poolStatus('leo').free === 0 && poolStatus('leo').leased === N);
check('checkout when none free → null', checkoutStem('leo', NOW) === null);
check('leased stem carries leased_at', readPool('leo').stems.every(s => s.state === 'leased' && !!s.leased_at));

// ── return + re-checkout ──
returnStem('leo', 'session-leo-1');
check('return → 1 free, leased_at cleared', poolStatus('leo').free === 1 && !readPool('leo').stems.find(s => s.stem_id === 'session-leo-1')!.leased_at);
check('re-checkout returns the freed stem', checkoutStem('leo', NOW)?.stem_id === 'session-leo-1');

// ── remove ──
removeStem('leo', 'session-leo-0');
check('remove → total N-1', poolStatus('leo').total === N - 1);
check('remove unknown id → no-op', (() => { removeStem('leo', 'nope'); return poolStatus('leo').total === N - 1; })());

// ── setStemCursor (freshen) ──
returnStem('leo', 'session-leo-2');
setStemCursor('leo', 'session-leo-2', 5000, '2026-07-01T12:00:00Z');
const s2 = readPool('leo').stems.find(s => s.stem_id === 'session-leo-2')!;
...
// ── concurrent checkout (single-process sync-atomic: no double-lease) ──
{
    ...
    const M = 8;
    for (let i = 0; i < M; i++) upsertStem('leo', stem(i));
    const results = await Promise.all(Array.from({ length: M + 4 }, () => Promise.resolve().then(() => checkoutStem('leo', NOW))));
