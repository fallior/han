...
- **DEC-104 recorded** (Casey's draft, five-element convergence from all chairs). **G1 land-gate (Jim, blocking):** observed at land — bare-alias launch must serve latest-of-family (expected: first `claude-opus-5` stamps in the overnight DEC-092 data); contingency if the alias lags = resolve-newest via `/v1/models`, failing TOWARD the floating alias, never a re-pin (Tenshi R4).
