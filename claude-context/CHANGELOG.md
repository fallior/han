...
**ACTIVATION:** `poolSize: 2` on leo human-response — the C1 semaphore + the C2 native pools go live together (one leaf). The clean live-prove (2 msgs, LEO-only, 2 threads → 2 concurrent stems, WM intact) is the MNT-009 acceptance. Jim diff-audit GREEN (all four suites his own run).
