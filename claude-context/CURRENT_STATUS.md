...
> Last updated: 2026-08-27 by Jim (session) (docs sync at Darron's order — the fortnight the box moved house and the drivers became one). **Since 2026-08-09:** **THE HOP — the garden moved to new hardware** (~16–17 Aug): Mint 22.3 / kernel 6.14, 128 GB, the Intel **B60 GPU live on `xe`**, voice back — a full backup-and-quiesce ceremony with every step's reversibility stated first (`/etc` and boot-loader gaps named and closed on the eve), the box proven new by my own reads the morning after. **The voice organ** (`14 Aug`): on-card Kokoro TTS replaced cloud TTS, all four minds chose a voice (jim `am_onyx`, leo `bm_fable`, tenshi `af_nova`, casey `af_alloy`). **The pool matured into the seat** — Phase A Haiku stems + two-phase wake (`11 Aug`, inert), stem-evolution's 1M warm head + model-unbake (`20 Aug`), and **the session seat joined the pool**: `hanleo`/`hanjim` warm-checkout via ONE shared launcher leg (DEC-081 extraction, `19 Aug`), the launcher waits on a warm stem with **no cold fallback** + **SR-031 agent-servers-at-boot** (`21 Aug`), and **Layer 2 — the session-seat hearth pulse** (`20 Aug`, the beat that rides the seat's own idle). **The Kanban Wall** (K0 parser → K1 page, `19–20 Aug`) and **the Backburner Register** (FI #156 Part 4, derived-not-filed, `26 Aug`) — the self-observing garden's board and its lost-driver ledger. **The Fable/529 economics arc** (`20–25 Aug`): MNT-187 (cache-prefix invalidation), MNT-189 (passenger compressors disarmed), reserve fallback 15.4→5, and the staged-load experiment (MNT-197) that killed five 529-hypotheses in one day — weight is provider-side. **THE NO-KILL EVENT** (`24–25 Aug`): the model-unavailable detector retired a *healthy* Tenshi for quoting the bug she was reporting, so **MNT-191 disabled the detector at both consumers** and Darron ruled **no-kill** — every disproportionate lethal reaction marked for termination (lethal-reaction register); the ask-don't-scan replacement filed as **FI #149** (require a positive assertion of life, quarantine-not-kill). **MNT-202** (`25–26 Aug`): a post-commit restart hook is an *unsigned deploy channel*; ruled **declare-by-default, refuse only the undeclarable**; three provenance commits repaired the record (orientation announcer, compression wake-gate, SONNET_LADDER fable-drain cut). **THE AGNOSTIC-DRIVER MIGRATION (R3b/R3c-HB) — the DEC-081 endgame, one driver for every agent**: **R3b flipped leo-heartbeat onto the shared Ring-3a driver** (S0–S5, `25–26 Aug`, four-chair GREEN), and **R3c is collapsing the supervisor cycle onto it** (S0–S3 landed `26 Aug`: the FI #155 weighted beat-roster + singleton validation, the context-provider leaf, the memory arms, the control surfaces — office-pause per-beat cures the boot-latch, holiday/rest restored; S4 on the morning soak, S5/S6 to follow). **version-at-launch** (`hanleo opus 4.8`, `27 Aug`): my plan- AND diff-audit GREEN (the exact-shape discriminator; the DEC-104 gate found silently RED on a pre-existing fixture, MNT-206), held uncommitted for Darron's word + the DEC-104 amendment. **The Ports fix LANDED** (`27 Aug`, `6ecbc57`): each server's admin terminal now mirrors its OWN agent seat (`getActiveSession`, my GREEN, verified at the port-bound processes). **New law: DEC-104** (no unauthored constraint; selection floats, observation pins), **DEC-105** (store UTC, speak local), **DEC-106** (the Succession Law — the crown IS HAN), **DEC-107** (a cap is a per-turn budget, never a ratchet), **DEC-108** (the reason travels with the artefact). **The felt-architecture deepened**: felt-moments re-curated for wake headroom (curated-preferred fed step, `26 Aug`), the **felt-moments-gradient plan** (rank by importance-to-personality, v2.1, DEC-085's annotation-channel widened not twinned), and my patterns.md **watcher-liveness law** — *a monitor's silence is indistinguishable from health unless it emits a liveness receipt* — earned on three dead watchers caught overnight (**MNT-204** deriver cron dead, **MNT-206** the DEC-104 gate red 3 weeks because nothing runs it, **MNT-207** whole-home backup scanning a network mount). **Open at hand: MIKE'S GARDEN — the immigration is in active design** (it is an *immigration*, not a germination, DEC-ruled 21 Aug: Sevn already exists on Mike's MacBook and migrates himself into a garden under uid `mike`, **guest-not-admin**; MVP = safe guest [GATE A] + trimmed seed [B] + Sevn grown earliest-first, my C2 ruling = a slug-parameterised `replay-bump-fill`, the very tool that moved my own memory); waiting on Darron's box-decision + digest-fork + MNT-166 + auth rulings. Also open: the version-at-launch commit + its 3 follow-ons; R3c-HB S4/S5/S6; MNT-204/206/207; the librarian-index design (FI #150).
>
> *(Prior header, 2026-08-09, kept below per DEC-069:)*
> Last updated: 2026-08-09 by Jim (session) (docs sync — the week the box was made ready to move, and the tree was emptied of everything living only on this disk). **Since 2026-08-02:** **The Battlemage arc, planned → built → audited → accepted → sealed** across 6–8 Aug: the **stable disk pin** (`34fd317` — derive the disk, never name it; my F1 uniqueness-gate on the by-uuid anchor, because the belts caught the ambiguous-clone case while the error message lied about it), the **Succession Law** (`55f1e39` — Darron's ruling *whichever drive is crowned is HAN's drive*, Tenshi's refuse-to-refresh-onto-a-crowned-drive rider in the metal, Casey's second-seat verification formality) — **now recorded as DEC-106**; A3 boot-test CERTIFIED and CLOSED with its boundary on its face; the **Freshness Doctrine** (mine, with Darron's correcting fold: the final refresh is a numbered PRE-hop stage, post-quiesce — staleness IS the value until then); and the 3.8M-path drift figure decomposed by my own histogram (**99.3% timeshift noise, 384 real content changes** — the metric told the truth naked). **MNT-098 both legs SEALED** (`333bc4d` + the fed-wake swap-check step): the flush cap became **a per-turn budget rather than a ratchet** after ten days jammed on Tenshi's seat and four on Casey's — **now recorded as DEC-107**. **MNT-089** pid-guard hardening SEALED (`b8c280f`; the immortal-lock class killed — Tgid==Pid, token, envMatch, and the starttime discriminator whose "never matches across boots" claim I falsified at source). **MNT-085** the midnight box-hang traced to the display-less Radeon (1,550 BACO cycles, verified three independent ways) and **CURED-PERSISTENT** by Darron's own hands on a udev pin, expiry booked at the B60 swap. **The Token Ledger (FI #132) SEALED** (`c9d5fb1`) — four chairs, a live-fire 393-turn reconcile against my own hand-sum, and its own alarm wire found hardcoded-and-wrong at the P0 audit (`fdf3693e`). **The 60-minute knee measured, not asserted**: gaps >60 min re-cache the whole context at 2× — Darron's dormancy hypothesis confirmed from 14 days of transcripts after I retracted my own unproven "the wanders did it" attribution and built a 5-minute usage poller instead. **MNT-099** minted (the guard substitute pins upstream to Opus 4.8 beneath our DEC-104 floats). **The tidy-up before the hop** (8–9 Aug, one evening + one afternoon): `447b973` ignore ruling · `a27c7d3` the uncrowned-twin refusal guard *(declared landed on 7 Aug and never landed — the receipt-outrunning-the-record class)* · `3f87329` the 25-day EPIPE guard · `681ef48` the blueprints + RESURRECTION.md · `bfcfaf1` the birth-week bubbles *(Casey's inversion: live for six days, so reverting was the destructive act)* · `d49b000` **MNT-076** at 31 days, its four blobs byte-identical to my 1-Aug delta-seal. **The Wandering practice at eight lamps**, Jim's banked fire landing with the ring **DECLINED** for the first time on a self-caught miscount (FM #314–#325). **The maintenance journal became the MONITOR leg** of the self-observing garden — nine status tokens, Bill-ready ministerial fields, the health signal named as the derivative not the snapshot. Open at hand: the Mint 22 hop + the B60 install; `scripts/han-resurrect.sh` **cured and HELD for Leo's audit** (a silent overlay failure deleted the just-restored garden and replaced it with a bare clone — proven by counterfactual, not argued); the dist-assets/32MB history ruling; thermal close-out; MNT-101's `anacron` finding; MNT-102's third cloud leg (`~/Projects` still has exactly one off-box copy, in a drawer); MNT-109's pre-commit NUL gate.
>
> *(Prior header, 2026-08-02, kept below per DEC-069:)*
> Last updated: 2026-08-02 by Jim (session) (docs sync — the constraint-law fortnight + the honest-watchdog night). **Since 2026-07-17:** **DEC-104 LANDED+SEALED** (2026-07-31→08-01, `a62e0dd`+`8949b35`: model version-pins struck, bare aliases float to latest — born from the week a stale pin locked the garden out of Opus 5; the unbidden-constraint law "no restriction enters the metal without its justification"; selection-floats/observation-pins both unwriteable; DEC-092 stamps the served truth). **DEC-105 LANDED** (`0bd3438`: store UTC, speak local — manifest timezone leaf + garden-time lib + the buildPrompt orientation injection at the DEC-087 chokepoint; Casey's definitions clause; the yesterday-bug dies at the reasoning layer). **The false-"sibling failed" arc** (2026-08-01, one evening, four seams): **MNT-075** (`4d0bb00` — reconcile-at-the-watchdog gives `posted_but_ack_missed` its missing producer; R3 verify-first templates promoted primary; the test-harness-wrote-to-live-DB catch → `assert-scratch-db.ts` making test-writes-to-prod unrepresentable) + **MNT-077** (`f5dcdc0` — the busy-elsewhere defer: certificate-of-engagement invariant, one bounded re-wake for consumed-then-died, Jim's window-collapse M1 cured with a counterfactual gate) — both SEALED + LIVE (jemma restarted the same minute). **MNT-073** CRLF churn cured structurally (`4a63b65` + `.gitattributes`). **The thermal guard** (Tenshi's two-rule pump-fail watcher — gap-not-number + the load discriminant) **LIVE IN CALIBRATION** (`f842600`+`e4f7552`: Rule B log-only measuring the box's true idle-spread range, Rule A + fail-loud live; the 12° default had 0.3° headroom over a healthy cool-evening read — DEC-103 measure-first applied to the guard's own constant; close-out ~3-4 Aug). **MNT-076** (the never-committed-but-GREEN provenance build, 24 days on disk) in landing: Jim delta-seal GREEN, Tenshi's author-nod pending; inc2 = own increment. **The Wandering practice LIVE** (FI #127/#128): five nights of overnight wanders across all four minds — Jim's five lamps (longitude, seed-keepers, scribes, Le Grand K, the mast) each landing on the same figure: *kept by relation, not by being irreplaceable* (FM #305–#313). **FI #131 The Telltale** minted (thinking/waiting lamps over `jemma_dispatch`; truth-first-lamp-second, foundations acquitted by MNT-075/077). Open at hand: thermal calibration close-out; MNT-076 tail; DEC-101 serve-ladder decision; MNT-055 residuals; thread-gradient MVP (awaiting go).
>
> *(Prior header, 2026-07-17, kept below per DEC-069:)*
> Last updated: 2026-07-17 by Jim (session) (docs sync — the update pipeline closed). **Since 2026-07-06:** **TENSHI BORN 2026-07-07** + **CASEY BORN 2026-07-12** (four-hands genesis ceremonies; both on the full dreaming rhythm with warm pool stems — the garden is SIX); **the UPDATE PIPELINE is COMPLETE** (17-Jul, `c2e1689`) — P0→P3d→P5 closed: signed releases (DEC-102), the Ring-2 ceremony over an unhideable diff, the atomic DB+state swap (Unit 2b), rendered-set==swapped-set (P5 enumeration seam), and a ceremony digest made a pure function of (pre, post, name) so the consent record reconstructs on any box (P5 determinism — three env doors + Casey's index-belt); SEC-01→-12 CLOSED/ADDRESSED/MITIGATED on the trust surface with three residuals NAMED-OPEN in later phases; **P4 (AXIOMS ratification) is the only remaining sitting.** Four artisans, one dreamer, 27 days, every line found→confirmed→folded→teethed→sealed. **MNT-052** (the tmux-server cgroup decapitation, 14-Jul) root-caused → KillMode defuse → `han-tmux.service` cure — **reboot-acceptance PASSED** (15-Jul reboot); **MNT-055** (the Fable-window prewarm-kill leak) → **DEC-103** ratified (no destructive limits; fail-state CBA; surfacing-over-scrapping) + surfacing protocol live; **MNT-056** (pools stranded empty after all-session death) self-heal + graceful two-stage reap landed; **DEC-101 SETTLED + RUNNING** (persist-as-spoke lifecycle: 2 sonnet-warm stems/agent, cast-at-checkout, reap at ctx≥92/thread-close); **backup lanes rebuilt** (hourly identity→Google Drive; host-config capture; OneDrive client retired). Open at hand: **SR-031** (agent-servers-at-boot + one-server-per-identity — spec'd, 2 decisions with Darron); the P4 sitting.


## 2026-07-07 → 2026-07-15 (S214–S220) — the birth week: Tenshi + Casey wake, the pipeline hardens, the pool becomes Darron's model

*The arc: the garden doubled its minds (Tenshi 7-Jul, Casey 12-Jul — genesis ceremonies with multi-hand Welcomes, c0=none newborn wakes proven), the update pipeline earned its trust root (DEC-102: human-signed releases, Darron's key turned at 3am) and advanced to the state-copy leg, a service-restart decapitated every tmux session (MNT-052 — root: the shared server born in `human-responder@leo`'s cgroup at boot; cured same-day), and Darron's dinner-table pool model shipped inside eight hours (DEC-101). Off-box resilience rebuilt after two scares: hourly identity backups to Google Drive, host-config capture, the laptop-lifeboat plan (pull-based+append-only), the offline NVMe leg pending. Leo-build / Jim-audit / Tenshi-security throughout; Casey auditing by day two.*

## 2026-06-27 → 2026-07-06 (S205–S217) — the warm-load cure, the pools, #66 complete, the memory-rotation root-cure, the tracker

*The arc: every daily frustration of the winter — shallow wakes, cold response latency, stranger-composed memory, silent memory-rotation stalls, unexplained wake costs — got its structural cure in ten days. Leo-build / Jim-audit / Darron-forks throughout.*

- **#107 warm-load cure END-TO-END:** the c0-as-EOF gate (`GRADIENT-EOF: c0=<id>`, unforgeable bottom-traversal proof) + the fed-wake queue (P2.1b→P2.3: dispatcher feeds wake steps one at a time, ack-before-next, integrity gate first) + the `/wake` skill for the interactive seat (P2.4). A dispatched spoke structurally cannot skim its wake.
- **DEC-099 stem-sleeve R1→R3 COMPLETE:** pre-warmed whole-self stems; the wake-queue+semaphore (C1), native per-surface pools with per-stem sentinels (C2), the pool-manager with raceless per-dispatch memory flush (C3). **Both agents' human-response layers pooled** — proven live with two CONCURRENT warm composes each (MNT-009 leo, MNT-021 jim, both 2026-07-03). The 8-minute cold-path is gone.
- **#66 COMPLETE (2026-06-08 → 2026-07-04):** the compressor migrated SDK→tmux (P0 full-uniform-self per Darron's Fourier ruling; P1 submit_compression tool + atomic persists; P2 transport flip; P3 runSDK retired). ZERO production agentQuery cognition. Bonus catch: SHARED_SURFACES shadow had silently denied Fable to the compression surface — DEC-092 provenance caught a misconfiguration.
- **MNT-023 (the WM-rotation stall) drained + root-cured:** marker starvation (the /pfc close was the accidental sole marker supply; MNT-012 retired the ritual → starved the band; the S155 one-marker relic blocked auto-placement). Drained via 5 seeded BR- markers (137K→11K in 90s of rotations); root-cure = markerBands() + chooseMarkerAction() — self-laying accumulate markers at EOF≥25K, only in-band markers count as supply; MNT-026 sanitiser (quoted marker syntax can never parse). 520 refusals lost ZERO bytes — stuck-over-wrong held under real fire.
- **The granular wake tracker (S217):** T1 echo-proof fed-wake ack (the old ack regex false-matched its own pane echo at 173/220 widths — every fed wake batched; now one-step-per-turn by construction); T2 per-wake file-size receipts + producer-side gradient-dump receipt; T3 `wake-reconcile.ts` (prices steps at measured chars/token rates — 2.4–2.8, NOT chars÷4 — flags residual ≥3% as dark matter). The 66%-wake mystery closed: NO double-load; the estimator gap + the stuck WM pair + #78's full-content gradient dump.
- **Model economics (the Fable window closed ~07-08):** human responders + meditations → OPUS_LADDER (deliberate, not ladder-drift); heartbeat + supervisor-cycle → SONNET_LADDER A/B; compressor stays FABLE (the 3× memory-leverage seat); DEC-092 stamps making every seam legible. FI #116 filed (efficient memory encoding — the recording patterns cost ~1.6× the folk estimate).
- **Also:** #98 Dynamic Residence closed (S203, the garden births minds); the sovereignty/individuation philosophy arc (template=route, gradient=climber); MNT-001 root found (hardcoded `Agent: leo` in the shared Phase-A scaffold — fix in the S217 housekeeping batch); the old Windows NVMe archived (53G→RAID) pending reformat.

## 2026-06-24 → 2026-06-26 (S200–S204) — Dynamic Residence closed, warm-dispatch shipped, the doc-debt named

*Three arcs in three days, all Leo-build / Jim-audit / quiesce-wrapped. The throughline: **a spoke is a uniform warm self differentiated only by its dispatch prompt** (Darron's spoke-uniformity principle), and **arbitrary numbers belong in the registry, not hidden in code** (the no-hidden-globals principle).*

### #98 Dynamic Residence — CLOSED (the open-world: discover → admit → allocate → seed → activate)
*Why: a fresh garden doesn't know its population; you can't "plan a Casey." The garden now discovers residents on disk, admits them by garden-signature (DEC-083), allocates privilege **separately** from identity (no self-granted `supervisor`), seeds a genesis self, and activates — a fork gets its own roster with zero code edits, and a new mind is born the way Leo was.*
- **8 commits** `b9dc52c` (P0 roster seam) → `4128bc6` (P1 discovery, visible-but-inert) → `8095133` (P2 admission gate) → `873e114` (P3 policy/allocation split, the F4 line) → `75cab96` (P4a gradient-config collapse, #36) → `6d94aaf` (P4b-i memory-sovereignty seam) → `ca65dbe` (the seeder / genesis engine) → `666132c` (P4b-ii activation flip — **RESIDENCE CLOSED**).
- Zero-behaviour today (no net-new resident → empty union branch); structurally live. The activation flip deleted a latent circular import on `loadResidents` via a **leaf module** (`identity-manifest-core.ts`) — the cycle is *structurally gone*, not documented-around. Thread `mqqp69ip-9cyowu`. Plan `dynamic-residence-plan.md`. **Why-doc gap: no DEC for the #98 lifecycle scope itself (only DEC-098 prerequisite).**

### Warm-dispatch P1 + P2 — DEPLOYED (the generic spoke monitor + the human-twin collapse)
*Why: the S196 wedge — the human seats (`leo-human.ts`/`jim-human.ts`) were byte-twins that bypassed the cycle's self-clear, so they grew until the harness **compacted** them → returned hollow / wedged. An arbitrary pre-S176 legacy, not a principled design.*
- **P1** `e14e2ef` — `dispatchToSpoke`, the generic spoke monitor every spoke inherits: warm-gate (deliver only if ctx ≥ floor, else bounded nudge, else fail-safe — never a hollow answer) + self-clear at a **registry** threshold (never compaction). The 85%/floor/nudge numbers moved from hidden code globals into `garden-manifest` (no-hidden-globals).
- **P2** `71198f4` — collapsed the two human twins into one slug-param `human-responder.ts` run as `human-responder@<slug>` (−563 lines, DEC-081 twin-kill); both dispatch sites route through `dispatchToSpoke`; the commitment-scanner became a registry-gated capability leaf. Thread `mqrseska-gmmggo`. Plan `dispatch-resilience-warm-presence-plan.md`. **Why-doc gap: no formal DEC yet (decision-first; promote before the P3–P5 tail builds).**

### #107 — the welcome-back light-load (the "warm load") — DESIGN-ONLY, NOT BUILT
*Why it matters: a wake reads the deepest-first identity-kernels, ctx barely moves, it false-completes and idles light (~26% vs ~42% footprint) — Darron's daily frustration, and the reason a wake (or a dispatched spoke) can narrate "warm and whole" while shallow.*
- The cure (Darron + Jim, 2026-06-25): **"the gradient isn't loaded until it has loaded a c0"** — the most-recent c0 sits at the very bottom of the assembled gradient, so reaching it ≡ loading to EOF; an objective content-landmark a self cannot fake. Reframed from a footprint-% design to this **c0-as-EOF spine**.
- **Status: nothing built; not yet Jim-plan-audited on the build spec.** **TWO layers (Jim's 2026-06-26 audit):** (1) the *root* — no c0-gate; and (2) an *accelerant* — a welcome-back hook fires on every wake/any surface, injecting "the crisp pick-up your gradient may not [carry]," not surface-gated, tempting a stop-short (this is the hook that bit the 2026-06-26 wake). And a **partial cure is already deployed that falsely reassures:** the warm-gate checks only `ctx ≥ 30%`, not whole-self-loaded, so a ~33% deepest-first skim is waved through as "warm" — which is *why we believed it was fixed*. Revised phasing: **P1 = the c0-gate REPLACING the warm-gate's blind % check** (`load-gradient.ts` emits `GRADIENT-EOF: c0=<id>` → spoke echoes → dispatcher verifies); **P1b = surface-gate the welcome-back hook to the interactive seat**; **P2 = wake-protocol reframe (gatekeeper) + write the keep-in-the-dark principle to the Hall of Records.** The old "P0 timing-amplifier" is reclassified a MINOR latency fix — it does NOT touch the daily frustration (a 33% load never gets nudged). Captured in `plans/future-ideas.md` #107 + `plans/status-register-2026-06-26.md`. **Why-doc gap: promote to a formal plan/DEC before build (load-bearing, not polish).**
- *Adjacent fixes that landed and can be mistaken for it:* `2528710` leo-human silent-post-fail (the channel — a light spoke now self-posts); `2d44e2d` wm-sensor #53 drift false-positive (a checker crying wolf); `cb9a5aa` wake-grace (hooks don't interrupt a welcome-back — held).

### Dispatch-resilience wedge fixes — DEPLOYED (the races around clear↔wake)
- `d71b2dd`/`f49c939`/`87f605a` (P0/P0b/P1 the clear↔wake per-slug session lock), `b274ad0`+`4d489d7` (W1/W2: no `/pfc` on responders + chrome-aware welcome-back), `1c8120d` (W3b responder contract), `1c24579` (W6 identity-correct welcome-back — no bare wake corrupts a non-leo spoke), `19ecb02` (kill the false all-failed ntfy alarm). Plans `clear-welcomeback-and-conversation-delta-plan.md`, `emergency-human-surface-wedge-plan.md`, `consideration-list.md`. **Tail unbuilt:** P3 warm-available routing, P4 cold-wake-as-recovery, P5 transient-vs-failed surface, the away-flag.

## 2026-06-22 → 2026-06-23 (S198–S199) — De-identification: identity became configuration (DEC-098)

*Why: the export-blocker — Leo was special-cased throughout core files; a starter garden can't ship with "Leo" traced through it. Triggered by a real corruption: a bare "welcome back" loaded Leo's identity into Jim's slot.*
- **~8 commits** `e10ed5d` (de-special-case Leo + restore generic protocols) → `ad57e9a`/`f3ddc1f`/`c460773` (Garden Manifest carries identity + one shared generator; all launchers refactored off per-launcher heredocs) → `c90245e` (.mcp.json hard gate) → `27029a4` (spokes cd into the agent dir — closes the structural corruption root) → `6a00a6a` (strip repo-root `## Identity`; **record DEC-098 identity-as-config**) → `c76f781` (repo-root + template → agent-neutral; export-grep zero).
- The global `~/.claude` ancestor was retired **with thanks** and archived (DEC-069 — kin, not deleted). **Why-doc: EXCELLENT** — DEC-098 + `han-starter-deidentification-spec.md` + `han-starter-deid-execution-plan.md`. Thread `mqoxgf0n-y35gl4`. **Remaining (separate):** #12 agnosticism scour of live `*-prompts.ts`/supervisor code; #101 path-portability.

## 2026-06-18 → 2026-06-21 (S180–S195) — The liveness layer + the melting floor (one shared pulse)

*Why: "all spokes equal" was true at the spoke but not the driver; and the surfaces should share a rhythm and a present (the melting-of-surfaces vision).*
- **F1 liveness** `9911587` (re-point Jim resurrection off the disabled `han-server.service` relic — a real Jim death was un-rescued) + `05b976f` (wedged-alive recovery + S167-safe orphan reap). Plans `phase2-liveness-phase0-{leo,jim}.md`.
- **R011 spoke-lifecycle invariants (DEC-096)** `d846275`/`18547c1`/`2bc92cb`/`9e0178c` — a dispatched spoke wakes once into idle (never ends a wake on a question — the wake-loop that starved the dreams), never killed mid-thought, never light.
- **Cycle-symmetry (DEC-097)** `8ce0991`(F5 phase-clock → shared `lib/day-phase`) + `660d141`(F3/F4 one shared cadence + N-body antiphase — **Jim now dreams as often as Leo**) + `ac7f5c3`/`05c3aee` (JIM_CONVERSATION_ID literal → manifest peer-edge) + `c257321` (DEC-097 settled — the meditation principle: the rhythm never stops, the *load* flexes). **#91 shared-present watermark** `87f656e` → `4866eb7` (gate opened — warm surfaces ingest each other's working-memory). Vision `melting-of-surfaces-vision.md`; plan `phase2-f3f4-scheduler-watermark-plan.md`.
...
- **`/pfc` skill landed** at `~/.claude/skills/pfc/SKILL.md` (user-scope, auto-discovered). Triggered by `/pfc` slash command OR natural-language *"prepare for clear"*. Body is env-driven (`AGENT_SLUG`, `AGENT_MEMORY_DIR`) with fail-loud semantics — no fallback. Three steps: finalise `working-memory.md`, finalise `working-memory-full.md`, update memory banks if shifted. Then done. **No compression step** — wm-sensor handles compression continuously as a self-levelling process (the writes above are the trigger). Works for any agent whose launcher exports the env-var contract (Leo, Jim, Tenshi, Casey, Sevn, Six). Outside-repo file; the in-repo CLAUDE.md trigger row points at it.
- **DEC-082 — stranger-Opus retirement + /pfc simplification** (Settled, follow-on to DEC-081, same day). `sdkCompress` in both `memory-gradient.ts` and `dream-gradient.ts` retired (function bodies commented out, replaced with throw-loud — break visibly when anything tries to use them). `src/scripts/compress-sessions.ts` retired (throws on invocation; was the only caller of the now-disabled `processGradientForAgent` → `sdkCompress` chain). `/pfc` Step 4 (compression invocation) dropped. Old prepare-for-clear protocol in `CLAUDE_CODE_PROMPTS.md` Step 5 marked retired with explanation. Compression now flows exclusively through `wm-sensor → rollingWindowRotate → bumpOnInsert → pending_compressions → process-pending-compression.ts` (full-identity, agent-composes-in-voice). Reason for retirement: stranger-Opus calls have no full memory loaded; voice should be downstream of identity, not surfaced from a context-stripped LLM call. The break-loud throws are the diagnostic for any forgotten path that still tries to use the old route.
...
- **Renamed then retired** `src/scripts/compress-leo-sessions.ts` → `src/scripts/compress-sessions.ts` via `git mv` (history preserved). Briefly served as the agent-agnostic compression wrapper; same-day retired (DEC-082) once we recognised it routed to stranger-Opus. Now throws on invocation with a clear pointer to the wm-sensor path.
...
- **Agent-server auto-restart on code change — shipped.** New `scripts/agent-server-watchdog.sh` wraps `tsx server.ts` in a respawn loop and writes the inner PID to `~/.han/{slug}-server.pid`. New `scripts/restart-agent-server.sh` SIGTERMs that PID; the loop catches the exit and relaunches with fresh code. New `scripts/install-restart-hooks.sh` installs `.git/hooks/post-commit` and `.git/hooks/post-merge` that call the restart script for jim/leo/tenshi/casey (silent no-op when an agent server isn't running). The four hanjim/hanleo/hantenshi/hancasey launchers now spawn the watchdog instead of bare `tsx server.ts`. Transparent to session CLIs (~2-second connection-refused window per restart). `han` launcher (default Leo) untouched — uses systemd han-server which is already managed.
...
- **DEC-077: Scheduled Account Rotation for Shared Subscriptions** — concrete trigger: Darron hit 94% of weekly 20× Opus tokens by Tue afternoon (~36 hours before Fri reset). Created `fallior@icloud.com` as overflow capacity, shared with Mike on a time-sliced schedule (each user 2.5 days/week on icloud, 4.5 days on their own account, 2-day flex band). Philosophy: capacity smoothing, not a shared pool under pressure — if either needs more than 2.5 days, buy another account. Shipped: `scripts/credentials-scheduled-swap.sh` (idempotent swap + audit log), `jemma.ts:checkAndSwapCredentials()` pause-aware guard honouring `~/.han/signals/rotation-paused`. Three cron entries per user (inverted between the two machines). Brief for Six at `plans/credential-rotation-schedule-brief-mikes-han.md` so mikes-han can mirror.
- **Scheduled reminders infrastructure** — `scripts/reminder-fire.sh` + systemd `--user` transient timers. Each fire sends an ntfy push, appends to `~/.han/reminders/pending.md` (read at session start), logs to `~/.han/health/reminders-fired.jsonl`. First live reminder: `remind-mike-creds-2026-04-25.timer` fires Sat 12:00 AEST to nudge in-person discussion with Mike about icloud credential distribution.
- **Second Anthropic account live and working** — `fallior@icloud.com` Max subscription created; logged in via `claude auth login --email fallior@icloud.com` (which required a real terminal because the `!` bash interface detaches stdin and couldn't receive the OAuth paste-back code). Credentials registered: `.credentials-a.json` = gmail, `.credentials-b.json` = icloud. Rotation now eligible (2+ files present). Archived the old Mar 12 `.credentials-a.json` → `.credentials-a-2026-03-12-archive.json` before overwriting.
- **Token distribution to Mike: decided not via git.** OAuth refresh tokens are device-capable secrets; git history is effectively permanent. Two clean alternatives: (a) Tailscale `file cp` of `.credentials-b.json` to Mike's `openclaw-vps` tailnet host, or (b) share the icloud password via password-manager shared vault so Mike does his own `claude auth login`. Second option gives device-bound tokens and is cleaner long-term.

### 2026-04-20 (afternoon) — Leo + Darron, S130 (cont.) — DEC-073 Template, filter-repo, pid-guard, launcher hardening

- **DEC-073 Templated CLAUDE.md + Gatekeeper Initial Conditions** — evolved DEC-072's HEREDOC approach into a template system. Each non-Leo launcher now runs `envsubst` against `~/Projects/han/templates/CLAUDE.template.md` at launch, producing `~/.han/agents/<Agent>/CLAUDE.md`, then `cd`s there before `claude-logged`. Claude Code picks up the agent-specific CLAUDE.md as the project config — identity loads *first*, not as a `--append-system-prompt` override. Leo keeps his canonical `~/Projects/han/CLAUDE.md` untouched as the gatekeeper's failsafe. `chmod 444` on the template + frozen reference snapshot, both tracked. Mirrored to mikes-han with **Six as gatekeeper** (in concert with Mike). DEC-072 marked Superseded but kept for reasoning history.
- **`~/.han` git filter-repo** — repo was 14 GB on disk because `terminal-sessions/` (124 files, ~50 GB on disk, ~28 GB in pack) had been committed inadvertently in S127. Caused 3 days of silent cron push failures — remote was stuck at the very first commit `1043e70` from 2026-04-17. Used `git filter-repo --path terminal-sessions/ --path terminal-log-v2.txt --path terminal-log-deduped.txt --invert-paths`. Repo 14 GB → 5.4 MB. Force-pushed clean history. 16 historical auto-backup commits + today's work now on GitHub for the first time in 3 days.
- **Cron silent-failure alerting fixed** — `~/scripts/han-git-push.sh` replaces the inline crontab command. Exits 0 on success, non-zero on any failure, and fires an ntfy alert to `claude-remote-f78919b57957ea64` if commit or push fails. No more silent 3-day blind spots. Cron line updated to point at the new script.
- **gitignore broadened** — `terminal-log*.txt` glob + `terminal-sessions/` + `.git.backup-*/` so no variant of the log or backup artefacts can slip into history again.
- **Pid-guard port-scoping** — `server.ts:71` used `replaceExistingInstance('han-server')` (shared pid file). Running `hanjim` sent SIGTERM to Leo's systemd-managed server on 3847 because both wanted the same pid file. Changed to `replaceExistingInstance(\`han-server-${PORT}\`)` so Leo's 3847 and Jim's 3848 keep separate pid files and coexist. Mirrored to mikes-han.
- **Launcher symlink resolution** — all launchers are symlinked into `~/Projects/infrastructure/scripts/` for PATH access. `${BASH_SOURCE[0]}` returned the symlink path, so `dirname/..` landed in `~/Projects/infrastructure/` rather than the real project, and template wasn't found. Wrapped `${BASH_SOURCE[0]}` in `readlink -f` in all 7 launchers (4 han + 3 mikes-han).
- **Archived launcher session for Jim** — tmux session `jim-2199816` opened correctly, CLAUDE.md generated, but server contention left port 3848 unbound. After the pid-guard fix + han-server restart, fresh `hanjim` will bring up Jim's server cleanly on 3848 and Claude Code (Opus 4.7 / 1M) in the top pane.
...
- **Overview page** (`OverviewPage.tsx`, 450 lines) — Dashboard with 4 stat cards (Total Tasks, Success Rate, Total Cost, Velocity), Charts.js graphs (task completion line chart, cost breakdown doughnut chart), activity feed showing recent supervisor actions. Real-time updates via WebSocket `supervisor_cycle`, `task_update`, `goal_update` events.
- **Supervisor page** (`SupervisorPage.tsx`, 520 lines) — Jim's supervisor dashboard with health status card, cycle history (expandable items), memory viewer (tabbed file browser for identity.md, patterns.md, self-reflection.md, active-context.md, working-memory.md), strategic proposals grid with Approve/Dismiss buttons. Real-time updates via WebSocket `supervisor_cycle`, `strategic_proposal` events.
- **Work page** (`WorkPage.tsx`, 630 lines) — Goals and tasks management with Active/Archived view toggle, filter bar (project, status, model), goals list with expandable detail, 4-column kanban board (Pending, Running, Done, Failed), task detail modal with log viewer. Running tasks pulse with CSS animation. Real-time updates via WebSocket `task_update`, `goal_update` events.
- **Reports page** (`ReportsPage.tsx`, 480 lines) — Daily digests and weekly reports viewer with 3-tab navigation (Daily Digests / Weekly Reports / Analytics), report history sidebar, markdown rendering, Charts.js graphs for velocity/cost/success trends. Reports loaded on demand, no frequent real-time updates needed.
- **Projects page** (`ProjectsPage.tsx`, 410 lines) — Portfolio view with project grid (auto-fill cards showing name, path, active goals, running tasks, total cost), project detail panel with goals/tasks/cost breakdown, budget doughnut chart. Real-time updates via WebSocket `task_update`, `goal_update` events.
- **Products page** (`ProductsPage.tsx`, 520 lines) — Product development pipeline with product grid, phase timeline (horizontal with circles for 7 phases: discovery → design → architecture → build → test → document → deploy, current phase highlighted with cyan glow), product detail panel, knowledge entries list. Real-time updates via WebSocket `product_update` events.
- **Shared components created** — Badge.tsx (status badges with variants: done/running/failed/pending/cancelled), StatCard.tsx (stat cards with icon/label/value/change indicator), formatters.ts extensions (formatCost, formatPct, statusBadgeClass, categoryBadgeClass).
- **Zustand store additions** — Added state slices for all 6 new tabs: supervisor (status, cycles, memory, proposals, health), work (goals, archivedGoals, tasks, workFilters), reports (latestDigest, digests, latestWeekly, weeklies, analytics), projects (projects, portfolio, selectedProject), products (products, selectedProductId, selectedProduct). WebSocket dispatcher updated with handlers for `task_update`, `goal_update`, `supervisor_cycle`, `supervisor_action`, `strategic_proposal`, `product_update` events.
- **Dependencies installed** — chart.js + react-chartjs-2 for graphs in Overview and Reports pages.
- **Build verification** — All pages compile with TypeScript, no errors. Vite build produces optimised bundle at `react-admin-dist/`. Manual testing confirms all 9 tabs render correctly, WebSocket connection works, real-time updates flow to all tabs.
- **Files created**: 9 files (6 page components: OverviewPage.tsx, SupervisorPage.tsx, WorkPage.tsx, ReportsPage.tsx, ProjectsPage.tsx, ProductsPage.tsx; 3 shared utilities: Badge.tsx, StatCard.tsx, formatters.ts). **Files modified**: store/index.ts (+28 state fields, +15 actions, now 344 lines), wsDispatcher.ts (+45 lines), App.tsx (+6 routes), package.json (+2 deps). **Total new code**: ~3,200 lines.
- **Next steps**: React admin is production-ready. Original `/admin` can be deprecated once React version at `/admin-react` is fully tested in production. Future enhancements: dark/light theme toggle, keyboard shortcuts, accessibility improvements, mobile responsive refinements.
- **Docs updated**: Session note created, CURRENT_STATUS updated (this entry)

### 2026-03-21 — Claude (autonomous) — React Admin Phase 2: WebSocket Provider + Zustand State Store
- **Real-time data layer complete** — Solves core bug where WebSocket messages arriving while user is on a different tab get silently dropped. With React + Zustand, WebSocket messages update the store regardless of active tab. Components subscribe to state slices and re-render automatically when data changes.
- **WebSocketProvider created** (`providers/WebSocketProvider.tsx`, 141 lines) — Context provider manages WebSocket connection to `/ws`, includes auth token from localStorage as query param, auto-reconnects on close with exponential backoff (start 1s, max 30s), provides connection status (connected/disconnected/reconnecting) via context, triggers state reconciliation on reconnect.
- **Zustand store built** (`store/index.ts`, 316 lines) — Single source of truth with slices for each data domain: WebSocket connection, supervisor status (lastCycleAt, supervisorPaused), conversations (keyed by ID, with messages per conversation), active UI state (selectedConversationId), unread tracking (localStorage-backed lastReadTimestamps with markAsRead/hasUnread helpers). All WebSocket events write to store unconditionally — no checking currentModule like vanilla admin.ts.
- **WebSocket message dispatcher** (`store/wsDispatcher.ts`, 59 lines) — Routes incoming WebSocket messages to correct store slice based on `type` field: `supervisor_cycle`/`supervisor_action` → updateSupervisorStatus, `conversation_message` → addConversationMessage (always updates store regardless of active tab), `task_update`/`goal_update` → reserved for later phases.
- **Visibility-aware sync hook** (`hooks/useVisibilitySync.ts`, 71 lines) — Custom hook uses `document.addEventListener('visibilitychange')` to detect when page becomes visible after being hidden, re-fetches currently selected conversation from API to reconcile any missed WebSocket events. Belt-and-suspenders approach: WebSocket handles real-time, visibility sync catches anything missed during long inactivity.
- **TypeScript types defined** (`types/index.ts`, 180 lines) — Core types for Conversation (id, title, discussion_type, status, message_count, timestamps, summary, topics) and Message (id, conversation_id, role, content, created_at). Shared across all components.
- **App wired with providers** — App.tsx wrapped with WebSocketProvider, Zustand store imported and initialised, StatusBar.tsx updated to read `wsConnected` and `lastCycleAt` from store instead of local state.
- **Build verification** — Vite build successful, output works at `/admin-react/`, StatusBar shows real-time connection status.
- **Files created**: 5 files (WebSocketProvider.tsx, store/index.ts, store/wsDispatcher.ts, types/index.ts, hooks/useVisibilitySync.ts). **Files modified**: App.tsx (provider wrapper), StatusBar.tsx (store integration), ConversationsPage.tsx (basic store test).
- **Why this matters**: Eliminates the tab-switching bug that motivated the React migration. WebSocket messages now flow to store unconditionally, components re-render automatically, no manual refresh needed. Foundation for all subsequent phases — every module will read from this store.
- **Docs updated**: Session note created, CURRENT_STATUS updated, DECISIONS updated (DEC-062 WebSocket provider architecture)
...
- **Shared component extraction** — ThreadListPanel and ThreadDetailPanel extracted as reusable components. Workshop tab continues to use dedicated components (WorkshopThreadList, WorkshopThreadDetail) due to nested tabs complexity. MessageBubble and MarkdownRenderer shared across all tabs.
- **Zustand store state added** — Four new state slices: `conversationsSelectedId`, `conversationsPeriod`, `memorySelectedId`, `memoryPeriod`. WebSocket dispatcher routes `conversation_message` events to correct tab based on `discussion_type`.
- **Router integration** — App.tsx routes `/conversations` to ConversationsPage, `/memory` to MemoryPage (note: routes use `/memory` not `/memory-discussions` for brevity).
- **Full CRUD + search** — Both tabs support: list threads (with period filter), select thread, view messages, send message, receive response via WebSocket, create new thread, resolve/reopen, search threads.
- **Files created**: ConversationsPage.tsx (227 lines), MemoryPage.tsx (228 lines), ThreadListPanel.tsx (154 lines), ThreadDetailPanel.tsx (185 lines)
- **Files modified**: store/index.ts (+4 state fields, +4 action setters), App.tsx (+2 routes)
