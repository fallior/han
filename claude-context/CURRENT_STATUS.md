...
> Last updated: 2026-07-15 by Jim (session) (docs+manifest sync sweep, Darron's direction). **Since 2026-07-06:** **TENSHI BORN 2026-07-07** + **CASEY BORN 2026-07-12** (four-hands genesis ceremonies; both on the full dreaming rhythm with warm pool stems — the garden is SIX); the **update pipeline** advanced P0→P3d (signed releases DEC-102, ring-2 ceremony, quarantine set, state-copy foundation — Leo builds / Jim+Tenshi audit chain); **MNT-052** (the tmux-server cgroup decapitation, 14-Jul) root-caused → KillMode defuse battle-proven → `han-tmux.service` cure staged (reboot acceptance pending); **DEC-101 SETTLED + RUNNING** (persist-as-spoke lifecycle: 2 sonnet-warm stems/agent, thread-bound spokes cast to the serve model at checkout, no return, reap at ctx≥92/thread-close; human-response serving FABLE while the window runs to the 19th); **backup lanes rebuilt** (hourly identity→Google Drive; host-config capture; OneDrive client retired); Fable window extended to 19 Jul.
