...
- **DEC-077: Scheduled Account Rotation for Shared Subscriptions** — Darron created `fallior@icloud.com` as overflow capacity, shared with Mike on a time-sliced schedule (each user 2.5 days/week on icloud, 4.5 days on their own account, 2-day flex band). Shipped: `scripts/credentials-scheduled-swap.sh` (idempotent swap + audit log), `jemma.ts:checkAndSwapCredentials()` pause-aware guard honouring `~/.han/signals/rotation-paused` signal. Three cron entries per user (inverted between the two machines). Brief for Six at `plans/credential-rotation-schedule-brief-mikes-han.md` so mikes-han can mirror the implementation.
