...
**Prohibited moves:** a warm ladder whose descent reaches Fable (re-arms the prewarm hang-loop); counting bound spokes against the pool's replenish target (`replenishPool` counts `.free` only — the stall-at-2 trap); returning a spoke to the free pool; reaping on any signal other than idle-ctx / thread-close / death (the spoke-cap + dormancy knobs live in the Odd-Jobs tweak register as FUTURE, with the leased_at-age reaper).
