...
| **Active Cascade** | **Retired 2026-05-17 per DEC-086.** Was: organic gradient deepening via random c1 selection + cascade promotion (10%/day + 5%/dream encounter). Produced 712 unhalted-INCOMPRESSIBLE entries promoted as same-size byte-shuffles at c8–c20 across both agents because no compression floor existed at depth after `ed8dfdc` removed it. Cascade is now exclusively insert-driven via `wm-sensor → bumpOnInsert → pending_compressions → process-pending-compression.ts` — the auto-levelling property of displacement covers what active-cascade was attempting. **The function body at `lib/memory-gradient.ts:623` is retained as recoverable infrastructure (retired by zero callers, not by throw)** so a manual emergency-cascade operation remains possible; adding a new caller is forbidden per the DO-NOT entry in CLAUDE.md. | retired |
...
The contradiction check runs as part of `bumpOnInsert()` (via the insert-driven cascade
chain), at the moment a new entry is about to be written. `bumpCascade()` was retired in
DEC-079 and `activeCascade()` was retired (zero callers) in DEC-086, so the check lives
solely on the insert-driven path now. Before writing the compressed entry or UV,
`checkUVContradiction()` checks existing entries at the target level and existing UVs for
semantic contradiction via Haiku. This ensures:
