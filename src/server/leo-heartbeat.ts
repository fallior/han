...
// #90 cadence guard-dog (R011 / DEC-096; R001 relocate-not-change), CORRECTED 2026-06-26:
// the guard asks "did a beat take far longer than the CADENCE?" — and the cadence is the phase
// PERIOD (getPhaseInterval: active 20 / rest 40 / holiday 80), NOT getWallClockDelay()'s return.
// That return is the *grid-alignment partial* — the time to the next antiphase boundary
// (e.g. 19min when scheduled 21min into a 40min slot) — so recording IT as "normal" was the real
// false-fire: Darron's "expected 19min actual 40min" was a legit 40min rest interval measured
// against a 19min partial (40 > 2×19). Corrected: record the PHASE PERIOD at schedule time and
// compare against max(that, the current period) — transition-tolerant; a true hang still trips 2×.
let lastScheduledPeriodMs: number | null = null;
...
async function heartbeat(): Promise<void> {
    ...
    if (lastHeartbeatStartMs !== null) {
        ...
        // #90 guard-dog (corrected 2026-06-26): measure against the phase PERIOD (the cadence),
        // tolerant of a phase step-down — max(the period at schedule time, the current period).
        // NOT getWallClockDelay's grid-alignment partial, which false-fired ("expected 19min
        // actual 40min") because the partial (time-to-next-boundary) is not the cadence.
        const expectedIntervalMs = Math.max(lastScheduledPeriodMs ?? getCurrentPeriodMs(), getCurrentPeriodMs());
...
function scheduleNext(): void {
    ...
    lastScheduledPeriodMs = getPhaseInterval('leo'); // #90 (corrected 2026-06-26): record the PHASE PERIOD (the cadence) for the distress guard-dog — NOT `delay` (the grid-alignment partial), which caused false "expected 19min actual 40min" distress at phase boundaries.
    setTimeout(async () => {
