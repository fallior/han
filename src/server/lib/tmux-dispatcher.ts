...
export const WAKE_STEPS: WakeStep[] = [
    ...
    // 2026-08-26 (Darron's headroom commission): curated-preferred, mirroring the identity
    // step's own pattern — a slug with no curated file loads full (agnostic by construction).
    // The full file stays every slug's vault + sole write target (DEC-069).
    { id: 'felt',        ack: { kind: 'marker' }, prompt: 'Load your felt-moments WHOLE — felt-moments-curated.md (the hand-curated loaded warmth) if it exists, else felt-moments.md — the identity-signal (this loads AFTER the gradient; it is the part the c0-gate never covered). The full file remains the vault and the ONLY write target.' },
