...
function renderMarkdown(text) {
    ...
    // Code blocks (``` ... ```)
    html = html.replace(/```(\w*)\n([\s\S]*?)```/g, (_m, _lang, code) => `<pre style="background:var(--bg-input);padding:10px;border-radius:6px;overflow-x:auto;font-size:12px;margin:8px 0"><code>${code.trim()}</code></pre>`);
    // Inline code
...
function renderModule(mod) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            switch (mod) {
                case 'overview':
                    yield loadOverview(content);
                    break;
                case 'projects':
                    yield loadProjects(content);
                    break;
                case 'work':
                    yield loadWork(content);
                    break;
                case 'supervisor':
                    yield loadSupervisor(content);
                    break;
                case 'reports':
                    yield loadReports(content);
                    break;
                case 'products':
                    yield loadProducts(content);
                    break;
                case 'conversations':
                    yield loadConversations(content);
                    break;
                case 'memory-discussions':
                    yield loadMemoryDiscussions(content);
                    break;
                case 'workshop':
                    yield loadWorkshop(content);
                    break;
            }
        }
        catch (err) {
...
function updateStatusInfo(data) {
    ...
    if (!el)
        return;
    if ((data === null || data === void 0 ? void 0 : data.type) === 'supervisor_cycle') {
...
function updateProposalBadge() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/supervisor/proposals`);
...
function loadOverview(content) {
    return __awaiter(this, void 0, void 0, function* () {
        const [analyticsRes, ecosystemRes, supervisorRes, activityRes] = yield Promise.all([
            fetch(`${API_BASE}/api/analytics`),
            fetch(`${API_BASE}/api/ecosystem`),
            fetch(`${API_BASE}/api/supervisor/status`),
            fetch(`${API_BASE}/api/supervisor/activity?limit=20`),
        ]);
        const analytics = yield analyticsRes.json();
        const ecosystem = yield ecosystemRes.json();
        const supervisor = yield supervisorRes.json();
        const activity = yield activityRes.json();
        ...
        createChart('velocityChart', {
            type: 'line',
            data: {
                labels: dailyCounts.map((d) => { var _a; return ((_a = d.date) === null || _a === void 0 ? void 0 : _a.slice(5)) || ''; }),
                datasets: [{
                        label: 'Tasks',
        ...
        const modelColors = modelLabels.map(m => m === 'opus' ? chartColor('purple') :
...
function renderActivityItems(events) {
    if (events.length === 0)
        return '<p style="color:var(--text-muted);font-size:13px;padding:12px">No recent activity</p>';
    return events.map(ev => {
        ...
        let detail = '';
        if (ev.detail) {
            if (ev.detail.observations)
                detail += `Observations: ${ev.detail.observations}\n`;
            if (ev.detail.actions)
                detail += `Actions: ${ev.detail.actions}\n`;
            if (ev.detail.reasoning)
                detail += `Reasoning: ${ev.detail.reasoning}\n`;
            if (ev.detail.error)
                detail += `Error: ${ev.detail.error}\n`;
            if (ev.detail.cost_usd)
                detail += `Cost: ${formatCost(ev.detail.cost_usd)}`;
        }
...
let workData = null;
function loadWork(content) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        const [tasksRes, activeGoalsRes, archivedGoalsRes] = yield Promise.all([
            fetch(`${API_BASE}/api/tasks`),
            fetch(`${API_BASE}/api/goals?view=active`),
            fetch(`${API_BASE}/api/goals?view=archived`),
        ]);
        const tasksData = yield tasksRes.json();
        const activeGoalsData = yield activeGoalsRes.json();
        const archivedGoalsData = yield archivedGoalsRes.json();
        ...
        const projects = [...new Set(tasks.map((t) => { var _a; return ((_a = t.project_path) === null || _a === void 0 ? void 0 : _a.split('/').pop()) || 'unknown'; }))].sort();
        ...
        for (const status of statuses) {
            ...
            for (const task of statusTasks) {
                const projectName = ((_a = task.project_path) === null || _a === void 0 ? void 0 : _a.split('/').pop()) || '—';
        ...
        if (activeGoals.length > 0) {
            ...
            for (const goal of activeGoals) {
                const proj = ((_b = goal.project_path) === null || _b === void 0 ? void 0 : _b.split('/').pop()) || 'unknown';
                if (!goalsByProject[proj])
                    goalsByProject[proj] = [];
        ...
        if (statusSelect)
            statusSelect.value = workFilters.status;
        if (projectSelect)
            projectSelect.value = workFilters.project;
        if (modelSelect)
            modelSelect.value = workFilters.model;
    });
}
function filterWorkTasks(tasks, statusFilter) {
    return tasks.filter(t => {
        ...
        const taskProject = ((_a = t.project_path) === null || _a === void 0 ? void 0 : _a.split('/').pop()) || '';
        const taskModel = t.model || '';
        let statusMatch = false;
        if (statusFilter === 'pending')
            statusMatch = taskStatus === 'pending';
        else if (statusFilter === 'running')
            statusMatch = taskStatus === 'running' || taskStatus === 'active' || taskStatus === 'decomposing';
        else if (statusFilter === 'done')
            statusMatch = taskStatus === 'done' || taskStatus === 'completed';
        else if (statusFilter === 'failed')
            statusMatch = taskStatus === 'failed';
...
window.applyWorkFilters = function () {
    ...
    const projectSelect = document.getElementById('filterProject');
    const modelSelect = document.getElementById('filterModel');
    workFilters.status = (statusSelect === null || statusSelect === void 0 ? void 0 : statusSelect.value) || '';
    workFilters.project = (projectSelect === null || projectSelect === void 0 ? void 0 : projectSelect.value) || '';
    workFilters.model = (modelSelect === null || modelSelect === void 0 ? void 0 : modelSelect.value) || '';
        ...
        if (content) {
            ...
            for (const status of columns) {
                ...
                if (col) {
                    ...
                    if (countEl)
                        countEl.textContent = String(statusTasks.length);
                }
            }
        }
    }
...
function loadProjects(content) {
    return __awaiter(this, void 0, void 0, function* () {
        const [ecosystemRes, portfolioRes] = yield Promise.all([
            fetch(`${API_BASE}/api/ecosystem`),
            fetch(`${API_BASE}/api/portfolio`),
        ]);
        const ecosystem = yield ecosystemRes.json();
        const portfolio = yield portfolioRes.json();
        const projects = ecosystem.projects || [];
        const portfolioMap = {};
            ...
            createChart('budgetChart', {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Spent Today',
                            data: budgetData.map((p) => { var _a; return ((_a = portfolioMap[p.name]) === null || _a === void 0 ? void 0 : _a.cost_spent_today) || 0; }),
                        ...
                        {
                            label: 'Daily Budget',
                            data: budgetData.map((p) => { var _a; return ((_a = portfolioMap[p.name]) === null || _a === void 0 ? void 0 : _a.cost_budget_daily) || 0; }),
    ...
    detail.innerHTML = `<div class="detail-panel fade-in">
        ...
        <div class="detail-grid">
            ...
            <div class="detail-field">
                ...
                <span class="value">${(pf === null || pf === void 0 ? void 0 : pf.priority) || 5}/10</span>
            </div>
            <div class="detail-field">
            ...
            <div class="detail-field">
                ...
                <span class="value">${(pf === null || pf === void 0 ? void 0 : pf.cost_budget_daily) ? formatCost(pf.cost_budget_daily) : 'None'}</span>
            </div>
            <div class="detail-field">
                <span class="label">Spent Today</span>
                <span class="value">${formatCost((pf === null || pf === void 0 ? void 0 : pf.cost_spent_today) || 0)}</span>
            </div>
            <div class="detail-field">
                <span class="label">Throttled</span>
                <span class="value" style="color:${(pf === null || pf === void 0 ? void 0 : pf.throttled) ? 'var(--red)' : 'var(--green)'}">${(pf === null || pf === void 0 ? void 0 : pf.throttled) ? 'Yes' : 'No'}</span>
            </div>
        ...
        ${(pf === null || pf === void 0 ? void 0 : pf.throttled) ? `<div style="margin-top:12px"><button class="admin-btn admin-btn-primary admin-btn-sm" onclick="unthrottleProject('${escapeHtml(proj.name)}')">Unthrottle</button></div>` : ''}
    </div>`;
}
window.unthrottleProject = function (name) {
    return __awaiter(this, void 0, void 0, function* () {
        yield fetch(`${API_BASE}/api/portfolio/${encodeURIComponent(name)}/unthrottle`, { method: 'POST' });
        renderModule('projects');
    });
};
// ══════════════════════════════════════════════════════════════
// MODULE: Supervisor
// ══════════════════════════════════════════════════════════════
function loadSupervisor(content) {
    return __awaiter(this, void 0, void 0, function* () {
        const [statusRes, cyclesRes, memoryRes, proposalsRes] = yield Promise.all([
            fetch(`${API_BASE}/api/supervisor/status`),
        ...
        const cyclesData = yield cyclesRes.json();
        const memoryData = yield memoryRes.json();
        const proposalsData = yield proposalsRes.json();
...
window.approveProposal = function (id) {
    return __awaiter(this, void 0, void 0, function* () {
        yield fetch(`${API_BASE}/api/supervisor/proposals/${id}/approve`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' }
        });
        renderModule('supervisor');
    });
};
window.dismissProposal = function (id) {
    return __awaiter(this, void 0, void 0, function* () {
        const notes = prompt('Dismiss reason (optional):');
        yield fetch(`${API_BASE}/api/supervisor/proposals/${id}/dismiss`, {
            method: 'POST',
...
window.triggerSupervisorCycle = function () {
    return __awaiter(this, void 0, void 0, function* () {
        yield fetch(`${API_BASE}/api/supervisor/trigger`, { method: 'POST' });
    });
};
window.toggleSupervisorPause = function () {
    return __awaiter(this, void 0, void 0, function* () {
        yield fetch(`${API_BASE}/api/supervisor/pause`, { method: 'POST' });
...
window.expandCycle = function (row, dataJson) {
    ...
    try {
        ...
        if (c.reasoning)
            detailHtml += `<div style="margin-bottom:8px"><strong style="color:var(--text-dim);font-size:11px;text-transform:uppercase">Reasoning</strong><div style="font-size:12px;color:var(--text);margin-top:4px;white-space:pre-wrap">${escapeHtml(c.reasoning)}</div></div>`;
        try {
        ...
        if (c.error)
            detailHtml += `<div style="margin-top:8px;color:var(--red);font-size:12px">${escapeHtml(c.error)}</div>`;
        detailHtml += '</td>';
...
// MODULE: Reports
// ══════════════════════════════════════════════════════════════
function loadReports(content) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        const [digestRes, digestsRes, weeklyRes, weekliesRes, analyticsRes] = yield Promise.all([
        ...
        const digestData = yield digestRes.json();
        const digestsData = yield digestsRes.json();
        const weeklyData = yield weeklyRes.json();
        const weekliesData = yield weekliesRes.json();
        const analytics = yield analyticsRes.json();
        // Header actions
        const actionsEl = document.getElementById('moduleActions');
        if (actionsEl) {
        ...
        if (suggestions.length > 0) {
            ...
            for (const s of suggestions) {
                ...
                <strong>${escapeHtml(((_a = s.project) === null || _a === void 0 ? void 0 : _a.split('/').pop()) || '')}</strong> (${escapeHtml(s.taskType)}):
...
window.generateDigest = function () {
    return __awaiter(this, void 0, void 0, function* () {
        yield fetch(`${API_BASE}/api/digest/generate`, { method: 'POST' });
        setTimeout(() => renderModule('reports'), 1000);
    });
};
window.generateWeekly = function () {
    return __awaiter(this, void 0, void 0, function* () {
        yield fetch(`${API_BASE}/api/weekly-report/generate`, { method: 'POST' });
        setTimeout(() => renderModule('reports'), 1000);
    });
};
window.viewDigest = function (id) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/digest/${id}`);
            const data = yield res.json();
            if (data.digest) {
                const content = document.getElementById('mainContent');
                if (!content)
...
function loadProducts(content) {
    return __awaiter(this, void 0, void 0, function* () {
        if (selectedProductId) {
            yield loadProductDetail(content, selectedProductId);
        }
        else {
            yield loadProductList(content);
        }
    });
}
function loadProductList(content) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield fetch(`${API_BASE}/api/products`);
        const data = yield res.json();
...
function loadProductDetail(content, productId) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield fetch(`${API_BASE}/api/products/${encodeURIComponent(productId)}`);
        const data = yield res.json();
...
// ══════════════════════════════════════════════════════════════
function loadConversations(content) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Fetch grouped conversations by temporal period
            const res = yield fetch(`${API_BASE}/api/conversations/grouped`);
            const data = yield res.json();
                ...
                for (const conv of conversations) {
                    ...
                    if (participants.length > 0) {
                        const badges = participants.map((p) => {
                            if (p === 'human')
                                return '<span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:rgba(56,139,253,0.3);color:var(--blue);font-size:9px;line-height:16px;text-align:center;font-weight:600" title="Darron">D</span>';
                            if (p === 'supervisor')
                                return '<span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:rgba(163,113,247,0.3);color:var(--purple);font-size:9px;line-height:16px;text-align:center;font-weight:600" title="Jim">J</span>';
                            if (p === 'leo')
                                return '<span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:rgba(56,207,135,0.3);color:var(--green);font-size:9px;line-height:16px;text-align:center;font-weight:600" title="Leo">L</span>';
                            return '';
                        }).join('');
                    ...
                    if (conv.topics) {
                        ...
                        if (topics.length > 0) {
                            const topicBadges = topics.slice(0, 3).map((topic) => `<span class="topic-badge" style="display:inline-block;background:var(--bg-input);color:var(--text-body);font-size:10px;padding:2px 8px;border-radius:3px;margin-right:4px;cursor:pointer" title="Filter by: ${escapeHtml(topic)}">${escapeHtml(topic)}</span>`).join('');
            ...
            if (selectedConversationId) {
                yield renderConversationThread(selectedConversationId);
            }
        }
        catch (err) {
...
window.filterConversationsByPeriod = function (period) {
    return __awaiter(this, void 0, void 0, function* () {
...
function renderConversationThread(conversationId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations/${conversationId}`);
            const data = yield res.json();
...
window.selectConversationThread = function (conversationId) {
    return __awaiter(this, void 0, void 0, function* () {
        selectedConversationId = conversationId;
        yield renderConversationThread(conversationId);
        // Update active state
        ...
        if (layout)
            layout.classList.add('thread-selected');
    });
};
window.backToThreadList = function () {
    ...
    const layout = document.querySelector('.conversation-layout');
    if (layout)
        layout.classList.remove('thread-selected');
};
...
function createNewConversation(title) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (data.conversation) {
                ...
                yield renderModule('conversations');
            }
        }
        catch (err) {
...
window.sendConversationMessage = function (conversationId) {
    return __awaiter(this, void 0, void 0, function* () {
        const input = document.getElementById('messageInput');
        ...
        input.value = '';
        try {
            yield fetch(`${API_BASE}/api/conversations/${conversationId}/messages`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
window.performConversationSearch = function (query, event) {
    return __awaiter(this, void 0, void 0, function* () {
        // Debounce search
        ...
        if (!query || query.trim().length === 0) {
            ...
            yield loadConversations(document.getElementById('mainContent'));
            return;
        }
        if (clearBtn)
            clearBtn.style.display = 'inline-block';
        conversationSearchTimeout = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e, _f, _g;
            try {
                const res = yield fetch(`${API_BASE}/api/conversations/search?q=${encodeURIComponent(query)}&limit=50`);
                const data = yield res.json();
                if (!data.success) {
                ...
                for (const [convId, result] of uniqueConversations) {
                    const snippet = ((_a = result.matched_message) === null || _a === void 0 ? void 0 : _a.snippet) || ((_b = result.matched_message) === null || _b === void 0 ? void 0 : _b.content) || '';
                    ...
                    const roleColor = ((_c = result.matched_message) === null || _c === void 0 ? void 0 : _c.role) === 'human' ? 'var(--blue)' : ((_d = result.matched_message) === null || _d === void 0 ? void 0 : _d.role) === 'leo' ? 'var(--green)' : 'var(--purple)';
                    const roleLabel = ((_e = result.matched_message) === null || _e === void 0 ? void 0 : _e.role) === 'human' ? 'Darron' : ((_f = result.matched_message) === null || _f === void 0 ? void 0 : _f.role) === 'leo' ? 'Leo' : 'Jim';
                    ...
                    <div class="search-result-snippet" style="background:var(--bg-card);border:1px solid var(--border-card);border-radius:4px;padding:10px;margin-bottom:8px;font-size:12px;color:var(--text-dim);line-height:1.5;max-height:60px;overflow:hidden;text-overflow:ellipsis">
                        <div style="display:flex;gap:6px;margin-bottom:4px">
                            ...
                            <span style="color:var(--text-muted);font-size:10px">${formatTime((_g = result.matched_message) === null || _g === void 0 ? void 0 : _g.created_at)}</span>
                        </div>
...
window.clearConversationSearch = function () {
    return __awaiter(this, void 0, void 0, function* () {
...
window.resolveConversation = function (conversationId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fetch(`${API_BASE}/api/conversations/${conversationId}/resolve`, { method: 'POST' });
            yield renderModule('conversations');
        }
        catch (err) {
...
window.reopenConversation = function (conversationId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fetch(`${API_BASE}/api/conversations/${conversationId}/reopen`, { method: 'POST' });
            yield renderModule('conversations');
        }
        catch (err) {
...
// ══════════════════════════════════════════════════════════════
function loadMemoryDiscussions(content) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations/grouped?type=memory`);
            const data = yield res.json();
            const periods = data.periods || {};
            // Header actions
            const actionsEl = document.getElementById('moduleActions');
            ...
            if (conversations.length === 0) {
                ...
                for (const conv of conversations) {
                    ...
                    if (conv.topics) {
                        const topics = conv.topics.split(',').map((t) => t.trim()).filter(Boolean);
                        if (topics.length > 0) {
                            const topicBadges = topics.slice(0, 3).map((topic) => `<span style="display:inline-block;background:rgba(179,146,240,0.15);color:var(--purple);font-size:10px;padding:2px 8px;border-radius:3px;margin-right:4px">${escapeHtml(topic)}</span>`).join('');
            ...
            if (selectedMemoryDiscussionId) {
                yield renderMemoryThread(selectedMemoryDiscussionId);
            }
        }
        catch (err) {
...
function renderMemoryThread(discussionId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations/${discussionId}`);
            const data = yield res.json();
            const conversation = data.conversation;
            const messages = data.messages || [];
...
window.selectMemoryThread = function (discussionId) {
    return __awaiter(this, void 0, void 0, function* () {
        selectedMemoryDiscussionId = discussionId;
        yield renderMemoryThread(discussionId);
...
window.filterMemoryByPeriod = function (period) {
    return __awaiter(this, void 0, void 0, function* () {
...
function createNewMemoryDiscussion(title) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations`, {
                method: 'POST',
            ...
            if (data.conversation) {
                ...
                yield renderModule('memory-discussions');
            }
        }
        catch (err) {
...
window.sendMemoryMessage = function (discussionId) {
    return __awaiter(this, void 0, void 0, function* () {
        const input = document.getElementById('mdMessageInput');
        if (!input || !input.value.trim())
            return;
        ...
        input.value = '';
        try {
            yield fetch(`${API_BASE}/api/conversations/${discussionId}/messages`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ content, role: 'human' })
            });
...
window.performMemorySearch = function (query, event) {
    return __awaiter(this, void 0, void 0, function* () {
        if (mdSearchTimeout)
            clearTimeout(mdSearchTimeout);
        const clearBtn = document.getElementById('mdClearSearchBtn');
        if (!query || query.trim().length === 0) {
            if (clearBtn)
                clearBtn.style.display = 'none';
            yield loadMemoryDiscussions(document.getElementById('mainContent'));
            return;
        }
        if (clearBtn)
            clearBtn.style.display = 'inline-block';
        mdSearchTimeout = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e, _f, _g;
            try {
                const res = yield fetch(`${API_BASE}/api/conversations/search?q=${encodeURIComponent(query)}&limit=50&type=memory`);
                const data = yield res.json();
                if (!data.success) {
...
window.resolveMemoryDiscussion = function (discussionId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fetch(`${API_BASE}/api/conversations/${discussionId}/resolve`, { method: 'POST' });
            yield renderModule('memory-discussions');
        }
        catch (err) {
...
window.reopenMemoryDiscussion = function (discussionId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fetch(`${API_BASE}/api/conversations/${discussionId}/reopen`, { method: 'POST' });
            yield renderModule('memory-discussions');
        }
        catch (err) {
...
function loadWorkshop(content) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Fetch grouped conversations for the current nested tab (discussion_type)
            const res = yield fetch(`${API_BASE}/api/conversations/grouped?type=${workshopNestedTab}`);
            const data = yield res.json();
            const periods = data.periods || {};
            // Header actions
            const actionsEl = document.getElementById('moduleActions');
            ...
            if (currentThreadId) {
                yield renderWorkshopThread(currentThreadId);
            }
        }
        catch (err) {
...
window.switchWorkshopPersona = function (persona) {
    return __awaiter(this, void 0, void 0, function* () {
...
window.switchWorkshopNestedTab = function (tab) {
    return __awaiter(this, void 0, void 0, function* () {
...
window.selectWorkshopThread = function (threadId) {
    return __awaiter(this, void 0, void 0, function* () {
        workshopSelectedThread[workshopNestedTab] = threadId;
        yield renderWorkshopThread(threadId);
...
window.filterWorkshopByPeriod = function (period) {
    return __awaiter(this, void 0, void 0, function* () {
...
window.backToWorkshopThreadList = function () {
    workshopSelectedThread[workshopNestedTab] = null;
    const layout = document.querySelector('.workshop-conversation-layout');
    if (layout)
        layout.classList.remove('thread-selected');
};
window.showNewWorkshopThreadForm = function () {
    const title = prompt('Thread title:');
    if (!title)
        return;
    createNewWorkshopThread(title);
};
function createNewWorkshopThread(title) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations`, {
                method: 'POST',
...
function renderWorkshopThread(threadId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/conversations/${threadId}`);
            const data = yield res.json();
            const conversation = data.conversation;
            const messages = data.messages || [];
...
window.sendWorkshopMessage = function (threadId) {
    return __awaiter(this, void 0, void 0, function* () {
        const input = document.getElementById('workshopMessageInput');
        if (!input || !input.value.trim())
            return;
        ...
        input.value = '';
        try {
            yield fetch(`${API_BASE}/api/conversations/${threadId}/messages`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ content, role: 'human' })
            });
...
window.resolveWorkshopThread = function (threadId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fetch(`${API_BASE}/api/conversations/${threadId}/resolve`, { method: 'POST' });
            yield renderModule('workshop');
        }
        catch (err) {
...
window.reopenWorkshopThread = function (threadId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fetch(`${API_BASE}/api/conversations/${threadId}/reopen`, { method: 'POST' });
            yield renderModule('workshop');
        }
        catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (collapseBtn && layout) {
        ...
        if (localStorage.getItem('admin-collapsed') === '1')
    ...
    setInterval(() => __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/api/supervisor/status`);
            const data = yield res.json();
            const el = document.getElementById('statusInfo');
