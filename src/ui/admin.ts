    ...
    if (data.type === 'supervisor_cycle' || data.type === 'supervisor_action') {
        ...
        if (currentModule === 'workshop' && isWorkshopType) {
            ...
            if (data.conversation_id === currentThreadId) {
                ...
                // New message in same tab but different thread — refresh list
...
function renderHealthPanel(healthData: any): string {
    ...
    // Resurrection History (collapsible)
    if (resurrections && resurrections.length > 0) {
        ...
        // Show last 10 resurrections in reverse order (newest first)
...
(window as any).toggleHealthDetails = function() {
...
(window as any).toggleThreadPanel = function() {
