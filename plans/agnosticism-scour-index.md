...
**Phase 1 — Safe shared-infra DEBT (do anytime — no liveness/scheduler risk): ✅ SUBSTANTIALLY COMPLETE (S178–S179, 2026-06-15).**
...
5. ⏸️ **Low-risk tail — DEFERRED (ground-traced S179, value runs thin / mostly leave-it):**
   - **jemma `deliverToJim`/`deliverToLeo` (`:445/480` + `recipient:'jim'/'leo'` :452/489)** — **DEAD** (grep-zero callers; superseded by `deliverToPersona` per `:714`). The recipient literals are scope-correct dead code → **leave** (DEC-069 recoverable); a future tidy may retire the wrappers but there's no live debt.
   - **`conversations.ts:44` `supervisor→jim`** — documented HAN-bootstrap carve-out, planned for the persona-registry role↔slug map (Batch-7 Alt B) → **leave, not this scour.**
   - **`dream-gradient.ts` `agent: AgentName = 'leo'` ×6 (`:63/141/219/365/612` + getAgentDreamPaths)** — the only actionable micro-cure (remove the default, require the arg, kill the latent wrong-agent footgun). **Deferred — low value on largely-retired code:** `processDreamGradient`/`sdkCompress` are retired-by-throw (DEC-082) with their live callers commented out (`f38bfa1`); `sdkCompress`'s 4 internal calls lean on the default (removal needs threading `agent` through dead code); `readDreamGradient` is effectively uncalled. **Pick this up cheaply if/when dream-gradient is next touched for any reason** (clean-as-we-go), not as a standalone bite.
   *(Decision S179, Darron: go with "Phase-1 substantially complete + note the deferred micro-cure" rather than grind a low-value tail on dead code. The scour now sits at the Phase-1→Phase-2 boundary; Phase 2 is FENCED below.)*
